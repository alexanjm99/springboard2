...
#####
SELECT facid, name 
AS facility_name, membercost, monthlymaintenance 
FROM `Facilities` 
...
AS court_name, m.firstname|| ' '|| m.surname 
AS membername 
FROM `Bookings`AS b
...
#####
SELECT bf.name,m.firstname || ' ' || m.surname 
AS membername, bf.cost
...
ON b.facid = f.facid
LEFT JOIN `Members` AS m
ON b.memid = m.memid) AS inner_subq 
GROUP BY name) AS outer_subq
...
#####
SELECT m.surname || ', ' || m.firstname, n.surname || ', ' || n.firstname
FROM Members AS n
JOIN Members AS m
ON m.memid=n.recommendedby
...
#####
SELECT f.name, COUNT(DISTINCT(b.memid)) AS member_usage_count
FROM `Bookings` AS b
LEFT JOIN `Facilities` AS f
ON b.facid = f.facid
WHERE b.memid != 0
